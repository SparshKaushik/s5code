...
Clerk, APNs, and optional FCM credentials. Set `APNS_ENABLED=false` for an Android-only development deployment without Apple credentials. Production adopts the configured API and tunnel DNS zones
...
- `CLERK_SECRET_KEY`
- `APNS_PRIVATE_KEY`
- `FCM_SERVICE_ACCOUNT` or `FCM_PRIVATE_KEY` (with `FCM_PROJECT_ID` and `FCM_CLIENT_EMAIL`) when Android push is enabled
