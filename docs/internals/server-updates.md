...
There are two self-update shapes. Servers running from an npm tree use one stable launcher selected
by the platform service manager (systemd on Linux, launchd on macOS). Precompiled single-file
binaries replace their own executable (see [Release Binary Updates](#release-binary-updates)).
Foreground CLI processes do not self-update, and a running server never edits its service
definition or durable service state.
