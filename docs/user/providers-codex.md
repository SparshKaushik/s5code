...
This guide is for people who want to use more than one Codex account in S5 Code. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install S5 Code](./install.md).
...
In S5 Code Settings, name it something obvious:
...
In S5 Code Settings, add another Codex provider:
...
S5 Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, S5 Code treats it
