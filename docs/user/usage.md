...
**Usage** combines Codex, Claude Code, Grok Build, Cursor, OpenCode, and pi session history from your
...
variable; OpenCode follows `OPENCODE_DB` or `XDG_DATA_HOME`. Use absolute paths or `~/` paths in the
account's environment settings; relative
...
Rather than guess and quietly show you a wrong number, those rows can be tagged with a **Tag**
button. Tag it, and:
