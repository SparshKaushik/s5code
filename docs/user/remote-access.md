...
Use this when you want to connect to a S5 Code server from another device such as a phone, tablet, or separate desktop app.
...
run a headless server from the CLI, or have the desktop app launch S5 Code over SSH.
...
- HTTPS/WSS-compatible endpoints work from `https://app.s5code.touchtech.club`, but are not made the default
  automatically.
...
If the copied link points directly at `http://192.168.x.y:3773`, open it from a client that can reach that LAN address. If it points at `https://app.s5code.touchtech.club/pair?...`, the hosted web app will save the environment and connect directly to the backend URL in the link.
...
For `https://app.s5code.touchtech.club`, prefer an HTTPS Tailnet or other HTTPS endpoint. A plain `http://100.x.y.z:3773` endpoint can still work from a desktop client or another browser page served over HTTP, but it will not work from the hosted HTTPS app because of browser mixed-content rules.
...
Use this when you want the desktop app to start or reuse S5 Code on another machine over SSH.
...
The remote host must have a compatible Node.js runtime. S5 Code uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, S5 Code first checks whether `node` is on `PATH`. If it is missing, the launcher
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path S5 Code uses:
...
When the S5 Code web or desktop app and a remote server use different versions, a warning appears in
the conversation and in **Settings** → **Connections**. Follow the action shown there: S5 Code may
...
see [Keeping S5 Code in Sync](./updating.md).

On a Linux host, you can keep the server running after logout and manage it independently of the
connection method. See [Running S5 Code in the Background](./background-service.md).
...
The hosted web app at `https://app.s5code.touchtech.club` can save a remote backend in browser local storage from a URL like:

```text
https://app.s5code.touchtech.club/pair?host=https://backend.example.com:3773#token=PAIRCODE
```
...
Hosted pairing does not proxy traffic through S5 Code. The browser still connects directly to the backend URL in the pairing link.
