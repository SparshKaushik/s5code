...
This guide is for people who want to use more than one Claude setup in S5 Code. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install S5 Code](./install.md).
...
In S5 Code Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means S5 Code uses Claude Code's normal config directory.

When you set this field, S5 Code points Claude Code at that directory with the
...
`~/.claude_personal_home/.claude`, which is not where S5 Code looks.

Then add another Claude provider in S5 Code:
...
S5 Code only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so S5 Code keeps separate config directories isolated
...
Add or edit a Claude provider in S5 Code Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. S5 Code stores the value as a server secret and does not
...
S5 Code does not need a special Claude Code Router provider. Treat the router as a Claude
