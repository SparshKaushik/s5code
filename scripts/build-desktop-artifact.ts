...
const DESKTOP_APP_ID = "club.touchtech.s5code";
...
export const DESKTOP_FILE_EXCLUSIONS = [
  // S5 Code always passes the user's installed Claude executable to the SDK,
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "S5 Code (Nightly)"
    : (desktopPackageJson.productName ?? "S5 Code");
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    artifactName: "S5-Code-${version}-${arch}.${ext}",
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  ...
  Command.withDescription("Build a desktop artifact for S5 Code."),
