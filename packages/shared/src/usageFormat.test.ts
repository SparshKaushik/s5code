...
describe("hourly usage formatting", () => {
  ...
  it("builds an exact minute-aligned 24-hour request", () => {
    const window = makeWindow(1, [], "hour", new Date("2026-08-11T12:37:42.123Z"));
  ...
  it("degrades an unknown resolved zone to UTC instead of crashing", () => {
    ...
    try {
      ...
      expect(makeWindow(1, [], "hour", now).timeZone).toBe("UTC");
      expect(makeWindow(30, [], "day", now).timeZone).toBe("UTC");
    } finally {
