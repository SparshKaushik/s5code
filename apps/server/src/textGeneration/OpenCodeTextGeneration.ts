/**
 * OpenCodeTextGeneration — structured text generation for OpenCode (v2).
 *
...
import type { OpenCodeHostHandle } from "../provider/OpenCodeHost.ts";
...
export const makeOpenCodeTextGeneration = (
  hostHandle: OpenCodeHostHandle,
  _settings: OpenCodeSettings,
    ...
    const generatedTextResult = yield* Effect.tryPromise({
      try: async () => {
        ...
        try {
          ...
          const remove =
            (hostHandle.client.session as any)?.remove ??
            (hostHandle.client.session as any)?.delete;
          if (typeof remove === "function") {
            await remove.call(hostHandle.client.session, { sessionID: session.id }).catch(() => {});
          }
        }
      },
      catch: (cause) =>
        new TextGenerationError({
          ...
          detail: `OpenCode text generation failed: ${String(cause)}`,
          cause,
        }),
    });
