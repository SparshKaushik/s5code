/**
 * OpenCodeAdapter — provider adapter for OpenCode.
 *
 * Implements the ProviderAdapterShape SPI using OpenCode's event stream,
...
export function makeOpenCodeAdapter(
  ...
  return Effect.gen(function* () {
    ...
    const sessionsByThreadId = new Map<ThreadId, OpenCodeSessionContext>();
    ...
    const modelCatalogCache = new Map<string, OpenCodeCatalogModel>();

    const resolveCatalogModel = (
      directory: string,
      slug: string,
    ): Effect.Effect<OpenCodeCatalogModel | null> =>
      Effect.gen(function* () {
        ...
        for (const item of (modelsRes as { data?: any[] }).data ?? []) {
          if (item?.id) {
            ...
            const info: OpenCodeCatalogModel = { providerID: pId, modelID: mId, variants };
      ...
      yield* Stream.fromAsyncIterable(
        ...
        Stream.runForEach((rawEvent) =>
          Effect.gen(function* () {
            ...
            let parentContext: OpenCodeSessionContext | undefined;
    ...
    const forkThread = (
      ...
      Effect.gen(function* () {
        ...
        const targetContext: OpenCodeSessionContext = {
