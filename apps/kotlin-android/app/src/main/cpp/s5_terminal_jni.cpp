...
void FreeSession(Session* session) {
  ...
  if (session->key_encoder != nullptr) ghostty_key_encoder_free(session->key_encoder);
...
GhosttyKey SemanticKey(jint key) {
  if (key == 1) return GHOSTTY_KEY_BACKSPACE;
  if (key == 2) return GHOSTTY_KEY_ENTER;
  if (key == 3) return GHOSTTY_KEY_TAB;
  if (key == 4) return GHOSTTY_KEY_DELETE;
  if (key == 5) return GHOSTTY_KEY_END;
  if (key == 6) return GHOSTTY_KEY_HOME;
  if (key == 7) return GHOSTTY_KEY_INSERT;
  if (key == 8) return GHOSTTY_KEY_PAGE_DOWN;
  if (key == 9) return GHOSTTY_KEY_PAGE_UP;
  if (key == 10) return GHOSTTY_KEY_ARROW_DOWN;
  if (key == 11) return GHOSTTY_KEY_ARROW_LEFT;
  if (key == 12) return GHOSTTY_KEY_ARROW_RIGHT;
  if (key == 13) return GHOSTTY_KEY_ARROW_UP;
  if (key == 14) return GHOSTTY_KEY_ESCAPE;
  if (key == 15) return GHOSTTY_KEY_SPACE;
  if (key >= 100 && key < 126) {
    return static_cast<GhosttyKey>(GHOSTTY_KEY_A + key - 100);
  }
  if (key >= 200 && key < 210) {
    return static_cast<GhosttyKey>(GHOSTTY_KEY_DIGIT_0 + key - 200);
  }
  if (key >= 300 && key < 312) {
    return static_cast<GhosttyKey>(GHOSTTY_KEY_F1 + key - 300);
  }
  constexpr GhosttyKey punctuation[] = {
      GHOSTTY_KEY_BACKQUOTE, GHOSTTY_KEY_BACKSLASH, GHOSTTY_KEY_BRACKET_LEFT,
      GHOSTTY_KEY_BRACKET_RIGHT, GHOSTTY_KEY_COMMA, GHOSTTY_KEY_EQUAL,
      GHOSTTY_KEY_MINUS, GHOSTTY_KEY_PERIOD, GHOSTTY_KEY_QUOTE,
      GHOSTTY_KEY_SEMICOLON, GHOSTTY_KEY_SLASH,
  };
  if (key >= 400 && key < 411) return punctuation[key - 400];
...
Java_club_touchtech_s5code_kotlin_platform_terminal_GhosttyBridge_nativeCreate(
...
Java_club_touchtech_s5code_kotlin_platform_terminal_GhosttyBridge_nativeDestroy(JNIEnv*, jclass, jlong handle) {
  FreeSession(FromHandle(handle));
}

extern "C" JNIEXPORT jbyteArray JNICALL
Java_club_touchtech_s5code_kotlin_platform_terminal_GhosttyBridge_nativeFeed(JNIEnv* env, jclass, jlong handle,
                                                       jbyteArray data) {
...
extern "C" JNIEXPORT jbyteArray JNICALL
Java_club_touchtech_s5code_kotlin_platform_terminal_GhosttyBridge_nativeResize(
...
extern "C" JNIEXPORT void JNICALL
Java_club_touchtech_s5code_kotlin_platform_terminal_GhosttyBridge_nativeScroll(JNIEnv*, jclass, jlong handle,
                                                         jint rows) {
  auto* session = FromHandle(handle);
...
extern "C" JNIEXPORT void JNICALL
Java_club_touchtech_s5code_kotlin_platform_terminal_GhosttyBridge_nativeSetTheme(
...
Java_club_touchtech_s5code_kotlin_platform_terminal_GhosttyBridge_nativeSelectWordAt(JNIEnv*, jclass,
                                                               jlong handle, jint x,
                                                               jint y) {
  auto* session = FromHandle(handle);
...
extern "C" JNIEXPORT void JNICALL
Java_club_touchtech_s5code_kotlin_platform_terminal_GhosttyBridge_nativeExtendSelection(
...
extern "C" JNIEXPORT jboolean JNICALL
Java_club_touchtech_s5code_kotlin_platform_terminal_GhosttyBridge_nativeSelectAll(JNIEnv*, jclass,
                                                            jlong handle) {
  auto* session = FromHandle(handle);
  if (session == nullptr) return JNI_FALSE;
  std::lock_guard<std::mutex> lock(session->mutex);
...
extern "C" JNIEXPORT void JNICALL
Java_club_touchtech_s5code_kotlin_platform_terminal_GhosttyBridge_nativeClearSelection(JNIEnv*, jclass,
                                                                 jlong handle) {
  auto* session = FromHandle(handle);
  if (session == nullptr) return;
  std::lock_guard<std::mutex> lock(session->mutex);
...
extern "C" JNIEXPORT jbyteArray JNICALL
Java_club_touchtech_s5code_kotlin_platform_terminal_GhosttyBridge_nativeGetSelectionText(JNIEnv* env, jclass,
                                                                   jlong handle) {
  auto* session = FromHandle(handle);
...
extern "C" JNIEXPORT jbyteArray JNICALL
Java_club_touchtech_s5code_kotlin_platform_terminal_GhosttyBridge_nativeEncodeKey(
    JNIEnv* env, jclass, jlong handle, jint key, jint action, jint modifiers,
    jbyteArray text, jint unshifted_codepoint) {
  auto* session = FromHandle(handle);
  if (session == nullptr || session->key_encoder == nullptr) return env->NewByteArray(0);
  std::lock_guard<std::mutex> lock(session->mutex);
  ...
  ghostty_key_event_set_key(event, SemanticKey(key));
  ghostty_key_event_set_mods(event, static_cast<GhosttyMods>(modifiers));
  ghostty_key_event_set_unshifted_codepoint(
      event, static_cast<uint32_t>(std::max(unshifted_codepoint, 0)));

  std::vector<uint8_t> text_bytes;
  if (text != nullptr) {
    const auto text_length = env->GetArrayLength(text);
    text_bytes.resize(static_cast<size_t>(text_length));
    if (text_length > 0) {
      env->GetByteArrayRegion(text, 0, text_length,
                              reinterpret_cast<jbyte*>(text_bytes.data()));
  ...
  size_t written = 0;
  auto result = ghostty_key_encoder_encode(session->key_encoder, event, stack_buffer,
                                            sizeof(stack_buffer), &written);
  std::vector<uint8_t> encoded;
  if (result == GHOSTTY_SUCCESS) {
    encoded.assign(stack_buffer, stack_buffer + written);
  } else if (result == GHOSTTY_OUT_OF_SPACE && written > 0) {
    std::vector<char> dynamic_buffer(written);
    if (ghostty_key_encoder_encode(session->key_encoder, event, dynamic_buffer.data(),
                                   dynamic_buffer.size(), &written) == GHOSTTY_SUCCESS) {
      encoded.assign(dynamic_buffer.begin(), dynamic_buffer.begin() + written);
    }
  }
  ghostty_key_event_free(event);
  return ToJavaBytes(env, encoded);
}

extern "C" JNIEXPORT jbyteArray JNICALL
Java_club_touchtech_s5code_kotlin_platform_terminal_GhosttyBridge_nativeSnapshot(JNIEnv* env, jclass,
                                                           jlong handle) {
  auto* session = FromHandle(handle);
  if (session == nullptr) return env->NewByteArray(0);
  std::lock_guard<std::mutex> lock(session->mutex);
