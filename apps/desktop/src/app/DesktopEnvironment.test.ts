...
const defaultInput = {
  ...
  processArch: "arm64",
  appVersion: "0.0.22",
  appPath: "/Applications/S5 Code.app/Contents/Resources/app.asar",
  isPackaged: false,
  resourcesPath: "/Applications/S5 Code.app/Contents/Resources",
...
describe("DesktopEnvironment", () => {
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.appUserModelId, "club.touchtech.s5code.dev");
      assert.equal(environment.linuxWmClass, "s5code-dev");
      assert.deepEqual(
  ...
  it.effect("keeps implicit development state separate from production state", () =>
    Effect.gen(function* () {
      ...
      assert.equal(development.stateDir, "/Users/alice/.s5code/dev");
      assert.equal(production.stateDir, "/Users/alice/.s5code/userdata");
    }),
  );
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          T3CODE_DESKTOP_APP_USER_MODEL_ID: " club.touchtech.s5code.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "club.touchtech.s5code.dev.local");
    }),
  );
